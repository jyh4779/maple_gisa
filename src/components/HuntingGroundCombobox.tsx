    ...
    <div className="space-y-2">
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((name) => (
            <Badge key={name} variant="secondary" className="gap-1 pr-1 text-xs font-normal">
              {name}
              <button
                type="button"
      ...
      <Popover>
        <PopoverTrigger className="flex h-9 w-full items-center justify-between rounded-md border border-input bg-background px-3 text-sm hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <span className={value.length === 0 ? "text-muted-foreground" : ""}>
            {value.length > 0 ? `${value.length}개 선택됨` : "사냥터 검색 또는 선택..."}
          </span>
          <ChevronDown className="h-4 w-4 opacity-50" />
        </PopoverTrigger>
        <PopoverContent className="w-96 p-0" align="start">
          <Command>
            <CommandInput placeholder="사냥터 이름 검색..." />
            <CommandList>
              <CommandEmpty>검색 결과가 없습니다.</CommandEmpty>
              {HUNTING_GROUND_AREAS.map((area) => (
                <CommandGroup key={area} heading={area}>
                  {HUNTING_GROUNDS.filter((g) => g.area === area).map((g) => (
                    <CommandItem
                      key={g.name}
                      value={g.name}
